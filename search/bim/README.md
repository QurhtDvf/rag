...
$df_t$: 用語 $t$ を含む文書の数 (document frequency)、$N$: 全文書数、`pseudo_count_nr`: 非関連文書における擬似カウント（一般的には $0.5$ など）

$$
    P(t|nR) = \frac{df_t + \text{\texttt{pseudo\_count\_nr}}}{N + 2 \cdot \text{\texttt{pseudo\_count\_nr}}}
    $$
